...
const currentCtrInput = document.getElementById('current-ctr');

currentCtrInput.addEventListener('input', function() {
    let value = this.value;

    // 1. Удаление лишних символов и форматирование
    value = value.trim(); // Удаляем пробелы в начале и конце
    value = value.replace('%', ''); // Удаляем символ процента
    value = value.replace(',', '.'); // Заменяем запятую на точку

    // 2. Проверка на допустимые символы (цифры и одна точка)
    const validPattern = /^\d*\.?\d*$/; // Регулярное выражение: цифры, необязательная точка, цифры
    if (validPattern.test(value) || value === "") { // Разрешаем пустую строку (на случай, если пользователь стирает ввод)
        this.value = value; // Обновляем значение поля ввода отформатированным значением
    } else {
        // Если ввод не соответствует паттерну, можно:
        // 1. Отменить ввод последнего символа (сложнее реализовать точно)
        // 2. Восстановить предыдущее допустимое значение (нужно где-то хранить предыдущее значение)
        // 3. Просто не обновлять поле, и оставить некорректный ввод (менее желательно)
        // В этом примере просто ничего не делаем, оставляя предыдущий ввод. Можно добавить визуальную обратную связь, например, меняя цвет рамки поля.
        // console.warn("Некорректный ввод для CTR"); // Можно вывести предупреждение в консоль для отладки
         // Альтернативно, можно вернуть поле к предыдущему валидному значению (если вы его сохраняли)
         // this.value = this.dataset.previousValidValue || ""; // Пример, если вы сохраняли предыдущее значение в data-атрибуте
    }
     // Сохраняем текущее значение как "предыдущее валидное" для возможного восстановления
