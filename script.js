...
function getInputValue(id) {
  ...
  function displayResults(results) {
      ...
      const emptyHeaderCell = headerRow.insertCell(); // Пустая ячейка для выравнивания
      emptyHeaderCell.style.border = "none"; // Убираем границу у пустой ячейки
      ...
      metrics.forEach(metric => {
        ...
        // Добавляем данные для каждого CTR
        headerTitles.slice(1).forEach(ctrKey => { // Пропускаем "Показатель"
